...
export const authService = {
  ...
  getUserRole: () => {
   ...
   // Check if user has permission for a specific role (strict mode)
  ...
  /**
   * Check if user has permission for a specific role
   * Instead of hardcoded hierarchy, prioritize ROLE_PERMISSIONS
   */
  hasPermission: (requiredRole) => {
    const userRole = authService.getUserRole();
    if (!userRole) return false;

    // Ưu tiên ROLE_PERMISSIONS
    if (ROLE_PERMISSIONS[userRole]) {
      return ROLE_PERMISSIONS[userRole].roles?.includes(requiredRole);
    }

    // Nếu role chưa được định nghĩa trong ROLE_PERMISSIONS thì fallback về hierarchy
    const roleHierarchy = {
      'Admin': 6,
      'Manager': 5,
      'Sales': 4,
  ...
  /**
   * Check if user has a specific action permission (e.g., "edit", "view")
   */
  hasSpecificPermission: (permission) => {
    const userRole = authService.getUserRole();
    if (!userRole) return false;
